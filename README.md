...
> 참고: 백엔드는 `.env`가 없으면 SQLite fallback으로도 실행됩니다. 프론트엔드는 기본적으로 `http://localhost:8000`의 백엔드 API를 호출합니다.
...
SQLite fallback과 mock route만으로 백엔드를 먼저 확인할 경우 이 단계는 생략할 수 있습니다.
...
OSRM 서버에 연결할 수 없는 경우, 개발 편의를 위해 mock GeoJSON 경로가 반환됩니다.
...
- 자갈, 모래, 비포장 등 거친 표면에 패널티 적용
- asphalt, paved, concrete 같은 포장 도로 선호
